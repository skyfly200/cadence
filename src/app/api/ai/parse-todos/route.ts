...
function fallbackParse(text: string, defaultStatus?: string): ParsedTask[] {
  const lines = text
    .split(/\n/)
    .map((l) => l.trim())
    .filter((l) => l.length > 0);

  // Track indent-based nesting so sub-items become notes of the parent
  ...
  for (const line of lines) {
    ...
    const cleaned = line
      ...
      .replace(/^[\s]*\[[ xX]\]\s+/, '')      // markdown checkboxes
      .replace(/^[☑☒✓✗✔✘]\s+/, '')           // unicode checkboxes
      .replace(/^[-*•]\s+\[[ xX]\]\s+/, '')   // combined bullet + checkbox
      .replace(/^\d+[.)]\s+\[[ xX]\]\s+/, '') // combined number + checkbox
      .trim();

    if (cleaned.length === 0) continue;

    // Detect already-completed items (strikethrough, checked boxes)
    const isCompleted =
      (cleaned.startsWith('~~') && cleaned.endsWith('~~')) ||
      /^\[[xX]\]/i.test(line) ||
      /^[☑☒✓✗✔✘]/.test(line);

    // For completed items, keep them but note it
    const title = isCompleted
      ? cleaned.replace(/^~~|~~$/g, '').trim()
      : cleaned;

    // Extract due dates / context as notes
    let taskTitle = title;
    let notes: string | null = null;

    const dueDateMatch = taskTitle.match(
      /(?:due[:\s]*|by[:\s]*|before[:\s]*)([^\n,;]+)/i
    );
    ...
    if (/cook|gym|run|walk|doctor|dentist|meditat|yoga|sleep|stretch|workout/i.test(lower)) {
      category = 'Health';
    } else if (/design|draw|paint|write|blog|edit|video|photo|music|creat/i.test(lower)) {
      category = 'Creative';
    } else if (/clean|laundry|grocer|fix|repair|mow|vacuum|dish/i.test(lower)) {
      category = 'Maintenance';
    } else if (/read|learn|study|course|book|tutorial|practic/i.test(lower)) {
      category = 'Learning';
    } else if (/call|meet|email|text|lunch|coffee|dinner|visit|party|hang/i.test(lower)) {
      category = 'Social';
    }

    // Heuristic eisenhower
    let eisenhowerCategory: ParsedTask['eisenhowerCategory'] = 'schedule';
    if (/urgent|asap|emergency|immediate/i.test(lower)) {
      eisenhowerCategory = 'do_first';
    } else if (/maybe|someday|optional|nice\s*to\s*have|whenever/i.test(lower)) {
      eisenhowerCategory = 'eliminate';
    } else if (/call|email|text someone|delegate|ask|assign/i.test(lower)) {
...
export async function POST(req: NextRequest) {
  ...
  // Truncate extremely long input (>20k chars) to avoid token limits
  const inputText =
    text.length > 20_000
      ? text.slice(0, 20_000) + '\n\n[... truncated ...]'
      : text;

  // If the text is very short (just one line), skip LLM and use fallback
  const lines = inputText.split(/\n/).filter((l) => l.trim().length > 0);
  if (lines.length <= 1) {
    return NextResponse.json(fallbackParse(inputText, defaultStatus));
  }
...
Each element must match this exact TypeScript interface:
{
  ...
  "priority": 1 | 2 | 3 | 4 | 5
      ...
      if (Array.isArray(parsed)) {
        ...
        const sanitized: ParsedTask[] = parsed.map((item: Record<string, unknown>) => ({
          ...
          estimatedMinutes: Math.max(
            1,
            Math.min(1440, Math.round(Number(item.estimatedMinutes) || 30))
          ),
          category: categories.includes(item.category as (typeof categories)[number])
            ? (item.category as (typeof categories)[number])
            : 'Admin',
          eisenhowerCategory: eisenhowerCategories.includes(
            item.eisenhowerCategory as (typeof eisenhowerCategories)[number]
          )
            ? (item.eisenhowerCategory as (typeof eisenhowerCategories)[number])
            : 'schedule',
          priority: validPriorities.includes(item.priority as (typeof validPriorities)[number])
            ? (item.priority as (typeof validPriorities)[number])
            : 3,
        }));
    ...
    // LLM returned something we couldn't parse — fall back to line-based parsing
    console.error(
      'parse-todos: LLM response did not contain a valid JSON array. Falling back.'
    );
    return NextResponse.json(fallbackParse(inputText, defaultStatus));
  } catch (error) {
    console.error(
      'parse-todos: LLM call failed, using fallback parser.',
      error
    );
    return NextResponse.json(fallbackParse(inputText, defaultStatus));
  }
